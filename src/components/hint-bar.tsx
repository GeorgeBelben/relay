import { getGamepadIconUrl } from "@/lib/gamepad/icons";
import { ControllerType } from "@/lib/gamepad/types";
...
export function HintBar() {
    ...
    const controllerType: ControllerType = "switch"
        ...
        <div className="fixed inset-x-0 py-6 px-16 bottom-0 z-70 flex items-center justify-end gap-4 bg-linear-to-t from-[#111111] via-[#111111]/70 to-[#111111]/0">
            {hints.map((hint) => (
                <div key={hint.action} className="flex items-center gap-1.5">
                    <img
                        src={getGamepadIconUrl(controllerType, hint.action)}
                        alt=""
                        className="h-6 w-6"
                    />
